...
"""
from typing import Any, Dict, Type, get_type_hints, List as TypingList
...
class Integer(Validator):
    def validate(self, value: Any, field_name: str) -> int:
        if not isinstance(value, int):
            raise TypeError(f"Field '{field_name}' must be an integer, got {type(value).__name__}")
        return value
...
class Float(Validator):
    def validate(self, value: Any, field_name: str) -> float:
        ...
        if not isinstance(value, float):
            raise TypeError(f"Field '{field_name}' must be a float, got {type(value).__name__}")
        return value
...
class Boolean(Validator):
    def validate(self, value: Any, field_name: str) -> bool:
        if not isinstance(value, bool):
             raise TypeError(f"Field '{field_name}' must be a boolean, got {type(value).__name__}")
        return value
...
class Const:
    ...
    def validate(self, value: Any, field_name: str) -> Any:
        if value != self.const_value:
            raise ValueError(f"Field '{field_name}' must be '{self.const_value}', got '{value}'")
        return value
...
class Array(Validator):
    ...
    def validate(self, value: Any, field_name: str) -> list:
        if not isinstance(value, list):
            raise TypeError(f"Field '{field_name}' must be a list, got {type(value).__name__}")
...
def _validate_field(value: Any, type_hint: Any, field_name: str) -> Any:
    ...
    if isinstance(type_hint, type) and issubclass(type_hint, Validator) and type_hint is not Array:
    ...
    # 2. Struct (Nested)
        ...
        if isinstance(value, dict):
            ...
            return value
             ...
             raise TypeError(f"Field '{field_name}' must be dict or {type_hint.__name__}, got {type(value).__name__}")
    ...
    # 6. Fallback (Native types)
        ...
        if not isinstance(value, type_hint) and type_hint is not Any:
            raise TypeError(f"Field '{field_name}' expected {type_hint.__name__}, got {type(value).__name__}")
        return value
