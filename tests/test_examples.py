...
def run_example_test(script_name, port, tests):
    ...
    # Start process
    proc = subprocess.Popen([PYTHON, script_path], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        for path, expected_status, check_func in tests:
            ...
            try:
                ...
                if resp.status_code != expected_status:
                    print(f"[FAIL] {script_name} {path}: Expected {expected_status}, got {resp.status_code}")
                    return False
...
def main():
    # Hello World Tests
    hello_tests = [
        ("/", 200, lambda r: r.json()["message"] == "Hello, World!")
    ]
    ...
    data_tests_refined = [
        (("POST", "/echo/json", {"key": "value"}), 200, check_json)
    ]
