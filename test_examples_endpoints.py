...
body_depends_tests = [
    # Body validation
    ("Valid user creation", "POST", "http://localhost:5023/users", 
     {"name": "Alice", "email": "alice@example.com", "age": 30}, 
     {"Content-Type": "application/json"}, 200),
    
    ("Invalid user (bad email)", "POST", "http://localhost:5023/users",
     {"name": "Bob", "email": "invalid", "age": 25},
     {"Content-Type": "application/json"}, 400),
    
    ("Invalid user (age too high)", "POST", "http://localhost:5023/users",
     {"name": "Charlie", "email": "charlie@example.com", "age": 200},
     {"Content-Type": "application/json"}, 400),
    
    # Dependency injection
    ("Profile with valid token", "GET", "http://localhost:5023/profile?token=secret",
     None, {}, 200),
    
    ("Profile with invalid token", "GET", "http://localhost:5023/profile?token=invalid",
     None, {}, 401),
    
    ("Admin users with valid token", "GET", "http://localhost:5023/admin/users?token=secret",
     None, {}, 200),
    
    ("Admin users with user token", "GET", "http://localhost:5023/admin/users?token=user123",
     None, {}, 403),
    
    # Combined Body + Depends
    ("Admin create user", "POST", "http://localhost:5023/admin/users/create?token=secret",
