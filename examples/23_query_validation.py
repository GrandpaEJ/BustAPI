...
@app.route("/api/data")
def get_data(
    format: str = Query("json", regex=r"^(json|xml|csv)$", description="Response format"),
...
def analytics(
    ...
    granularity: str = Query("day", regex=r"^(hour|day|week|month)$", description="Data granularity")
):
    """
    Get analytics data.
...
if __name__ == "__main__":
    ...
    print("  curl 'http://localhost:5022/products?category=electronics&min_price=10&max_price=100'")
    ...
    print("  curl 'http://localhost:5022/users?role=invalid'  # role must match pattern")
    print("\n" + "=" * 60)
