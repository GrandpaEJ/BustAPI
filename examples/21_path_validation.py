...
def get_user(user_id: int = Path(ge=1, le=1000, description="User ID between 1 and 1000")):
    """
...
def get_product(product_id: int = Path(gt=0, description="Product ID must be positive")):
    """
    Get product by ID.
...
def get_tag(tag_name: str = Path(regex=r"^[a-z0-9-]+$", description="Tag name (lowercase alphanumeric with hyphens)")):
    """
...
def check_price(price: float = Path(ge=0.01, le=999999.99, description="Price between $0.01 and $999,999.99")):
    """
