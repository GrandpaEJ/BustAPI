...
POSTS = {
    1: {"id": 1, "user_id": 1, "title": "Hello World", "body": "First post!", "likes": 42},
    2: {"id": 2, "user_id": 1, "title": "BustAPI Rocks", "body": "So fast!", "likes": 128},
    3: {"id": 3, "user_id": 2, "title": "Python Tips", "body": "Use type hints", "likes": 55},
}
...
def search():
    ...
    if search_type in ("users", "all"):
        results["users"] = [u for u in USERS.values() if query.lower() in u["name"].lower()]
    
    if search_type in ("posts", "all"):
        results["posts"] = [p for p in POSTS.values() if query.lower() in p["title"].lower()]
