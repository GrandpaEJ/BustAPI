...
class BenchmarkRunner:
    ...
    def run_wrk_benchmark(self, duration: int = 30, connections: int = 100, threads: int = 4) -> Optional[Dict]:
    ...
    def run_python_benchmark(self, duration: int = 10, concurrent_requests: int = 100) -> Dict:
        """Run Python-based benchmark using requests."""
        print(f"Running Python benchmark for {duration}s with {concurrent_requests} concurrent requests...")
    ...
    def test_endpoint_functionality(self) -> Dict:
        ...
        for test_name, test_config in tests.items():
            try:
                response = requests.get(f"{self.base_url}{test_config['url']}", timeout=5)
                results[test_name] = {
    ...
    def run_memory_test(self) -> Dict:
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    if 'python' in proc.info['name'] and any('bustapi' in arg for arg in proc.info['cmdline']):
            ...
            def monitor_memory():
                for _ in range(10):  # 10 samples over 5 seconds
                    try:
                        total_memory = sum(proc.memory_info().rss for proc in server_processes)
    ...
    def run_comprehensive_benchmark(self) -> Dict:
        ...
        results['python_benchmark'] = self.run_python_benchmark(duration=15, concurrent_requests=100)
