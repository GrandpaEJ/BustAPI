...
APPS = {
    "bustapi": {"cmd": [sys.executable, str(ROOT / "apps" / "bustapi_app.py")], "url": "http://127.0.0.1:8001/hello"},
    "flask": {"cmd": [sys.executable, str(ROOT / "apps" / "flask_app.py")], "url": "http://127.0.0.1:8002/hello"},
    "fastapi": {"cmd": [sys.executable, str(ROOT / "apps" / "fastapi_app.py")], "url": "http://127.0.0.1:8003/hello"},
}


async def worker_request_limited(task_id: int, client: httpx.AsyncClient, url: str, target_requests: int, counter, stop_event: asyncio.Event):
...
async def run_load_duration(url: str, concurrency: int, duration: int):
    ...
    async with httpx.AsyncClient() as client:
        ...
        tasks = [asyncio.create_task(worker_request_limited(i, client, url, 10**9, counter, asyncio.Event())) for i in range(concurrency)]
...
async def run_load_requests(url: str, concurrency: int, target_requests: int):
    ...
    async with httpx.AsyncClient() as client:
        tasks = [asyncio.create_task(worker_request_limited(i, client, url, target_requests, counter, stop_event)) for i in range(concurrency)]
...
def run_target_requests(target: str, concurrency: int, target_requests: int):
    ...
    try:
        ...
        print(f"Running load: {concurrency} concurrency for {target_requests} total requests against {app['url']}")
        start = time.time()
        results = asyncio.run(run_load_requests(app["url"], concurrency, target_requests))
        elapsed = time.time() - start
        success, errors = results
        rps = success / elapsed if elapsed > 0 else 0.0
        print(f"{target} results: success={success}, errors={errors}, elapsed={elapsed:.2f}s, rps={rps:.2f}")
        return {"target": target, "success": success, "errors": errors, "elapsed": elapsed, "rps": rps, "concurrency": concurrency}
    finally:
        proc.terminate()
        try:
            proc.wait(3)
...
def run_target_duration(target: str, concurrency: int, duration: int):
    ...
    try:
        time.sleep(1.5)
        print(f"Running load: {concurrency} concurrency for {duration}s against {app['url']}")
        start = time.time()
...
def main():
    ...
    parser.add_argument("--target", default="all", choices=["all", "bustapi", "flask", "fastapi"])
    parser.add_argument("--concurrency", type=int, default=50)
    parser.add_argument("--duration", type=int, default=None, help="duration in seconds (mutually exclusive with --requests)")
    parser.add_argument("--requests", type=int, default=None, help="total number of requests to send (mutually exclusive with --duration)")
