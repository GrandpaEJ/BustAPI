                        ...
                        Ok(result) => {
                            ...
                            if let Ok(asyncio) = asyncio {
                                match asyncio.call_method1("iscoroutine", (&result,)) {
                                    Ok(is_coro) => {
                                        let is_coro_bool = is_coro.extract::<bool>().unwrap_or(false);
                                        tracing::debug!("Async handler result type: {}, is_coro: {}", result.bind(py).get_type().name().ok().map(|s| s.to_string()).unwrap_or("unknown".to_string()), is_coro_bool);
                                        
                                        if is_coro_bool {
                                            // Run coroutine
                                            if let Ok(loop_obj) = asyncio.call_method0("NewEventLoop") { // Try new loop? No get_event_loop
                                                 // ...
                                            }
                                            ...
                                            if let Ok(loop_obj) = asyncio.call_method0("get_event_loop")
                                            {
                                                    ...
                                                    .call_method1("run_until_complete", (&result,))
                                                {
                                                 ...
                                                 if let Ok(loop_obj) = asyncio.call_method0("new_event_loop") {
                                                     let _ = asyncio.call_method1("set_event_loop", (&loop_obj,));
                                                      if let Ok(awaited) = loop_obj
                                                            .call_method1("run_until_complete", (&result,))
                                                        {
